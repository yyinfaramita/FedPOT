...
def main(args):
    ...
    while True:
        print(", Iteration = %d, data size = %.2f/%.2f" % (commu_loop, args.train_size, args.claim_size))
        ...
        honest_rewards_list = get_reward(args, last_server_state,
                                         size_list, valloader, testloader,
        ...
        for f in range(args.users):
            ...
            fraud_model = train_fraud_attacker(args, channel, num_classes,
                                                    last_server_state,
                                                    realloader_list[f])
            fraud_model_list.append(fraud_model)

            honest_model = local_model_list[f]
            local_model_list[f] = fraud_model
            fraud_rewards_list = get_reward(args, last_server_state, size_list,
                                            valloader, testloader, channel,
                                            num_classes, local_model_list)
            local_model_list[f] = honest_model
        ...
        for attackers in attackers_list:
            local_model_list[attackers] = fraud_model_list[attackers]
        ...
        if commu_loop == args.global_loops:
            break
...
if __name__ == '__main__':
    ...
    parser.add_argument('--attack_inner_loops', type=int, default=5, help='local update loop')
    ...
    parser.add_argument('--attack_lr', type=float, default=0.0001, help='learning rate of attacker')
